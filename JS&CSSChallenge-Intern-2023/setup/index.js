...
if (cardContainer) {
    console.log("the button is here");
}
...
// create and display the first four cards
for (let i = loadedCardsCount; i < loadedCardsCount + 4; i++) {
    cardContainer.innerHTML += createCard(data[i]);
}
loadedCardsCount += 4;

// add event listener to the Load More button
loadMoreBtn.addEventListener("click", () => {
    // create and display the next four cards
    for (let i = loadedCardsCount; i < loadedCardsCount + 4; i++) {
        if (data[i]) {
            cardContainer.innerHTML += createCard(data[i]);
        }
    ...
    // hide the Load More button when all cards have been loaded
    if (loadedCardsCount >= data.length) {
...
function createCard(cardData) {
    ...
    // add the card source link
    const sourceLink = document.createElement("a");
    sourceLink.href = cardData.source_link;
    sourceLink.textContent = cardData.source_type;
    ...
    // add the card likes counter and heart icon
    // add the card likes counter and heart icon
    ...
    heartIcon.addEventListener("click", () => {
        ...
        likesCounter.textContent = cardData.likes;
    });
